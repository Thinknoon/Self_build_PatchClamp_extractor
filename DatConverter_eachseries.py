...
from conversion_utils import PLACEHOLDER, V_CLAMP_MODE, I_CLAMP_MODE, \
...
class DatConverter_each_series:
    def __init__(self, inFile, outFile, multipleGroupsPerFile=False, compression=True):
        ...
        for elem in generateList(multipleGroupsPerFile, self.bundle.pul):
            for group_index in elem:
                for series_index in group_index:
                    nwbFile = self._createFile()
                    device = self._createDevice()
                    nwbFile.add_device(device)
                    self.electrodeDict = DatConverter_each_series._generateElectrodeDict(elem)
                    electrodes = self._createElectrodes(device)
                    nwbFile.add_ic_electrode(electrodes)
                    for i in self._createAcquiredSeries(electrodes, series_index,group_index):
                        nwbFile.add_acquisition(i, use_sweep_table=True)

                    for i in self._createStimulusSeries(electrodes, series_index,group_index):
                    ...
                    if multipleGroupsPerFile:
                        ...
                        outFileFmt = f"{name}-{group_index.GroupCount}series_{series_index.SeriesCount}{suffix}"

                    with NWBHDF5IO(outFileFmt, "w") as io:
                        io.write(nwbFile, cache_spec=True)

    def _createStimulusSeries(self, electrodes, series_group,group):
        """
        ...
        for sweep in series_group:
            cycle_id = createCycleID([group.GroupCount, series_group.SeriesCount, sweep.SweepCount],
            ...
            for trace in sweep:
                ...
                if not len(stimset):
                    print(f"Can not yet recreate stimset {series_group.Label}")
                    continue
                ...
                electrodeKey = DatConverter_each_series._generateElectrodeKey(trace)
                electrode = electrodes[self.electrodeDict[electrodeKey]]
                gain = 1.0
                resolution = np.nan
                stimulus_description = series_group.Label
                ...
                description = json.dumps({"cycle_id": cycle_id,
                                            ...
                                            "series_label": series_group.Label,
                                            "sweep_label": sweep.Label},
                                            sort_keys=True, indent=4)

                ampState = DatConverter_each_series._getAmplifierState(self.bundle, series_group, trace)
                clampMode = DatConverter_each_series._getClampMode(ampState, cycle_id, trace)
    ...
    def _createAcquiredSeries(self, electrodes, series_group,group):
        """
    ...
    def _generateElectrodeDict(groups):
        ...
        Use DatConverter_each_series._generateElectrodeKey(trace) for generating the key, the
        ...
        for group in groups:
            for series in group:
                for sweep in series:
                    for trace in sweep:
                        key = DatConverter_each_series._generateElectrodeKey(trace)
    ...
    def _getAmplifierState(bundle, series, trace):
        ...
        if DatConverter_each_series._isValidAmplifierState(ampState):
            return ampState
    ...
    def _check(self):
        ...
        deviceString = DatConverter_each_series._formatDeviceString(self.bundle.amp[0][0])

        for series_index, series in enumerate(self.bundle.amp):
            for state_index, state in enumerate(series):

                # skip invalid entries
                if not DatConverter_each_series._isValidAmplifierState(state.AmplifierState):
                    continue

                if deviceString != DatConverter_each_series._formatDeviceString(state):
                    raise ValueError(f"Device strings differ in tree structure " +
                                     f"({deviceString} vs {DatConverter_each_series._formatDeviceString(state)} " +
    ...
    def _createFile(self):
        ...
        self.session_start_time = DatConverter_each_series._convertTimestamp(self.bundle.header.Time)
    ...
    def _createDevice(self):
        ...
        name = DatConverter_each_series._formatDeviceString(self.bundle.amp[0][0])
    ...
    def _getStartingTime(self, sweep):
        ...
        sweepTime = DatConverter_each_series._convertTimestamp(sweep.Time)
