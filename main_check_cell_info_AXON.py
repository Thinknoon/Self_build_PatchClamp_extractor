...
all_files = glob.glob('LIP_AXON/*/*.nwb')
if os.path.exists('LIP_AXON/cell_metadata.csv'):
    pass_check_cell_meta = pd.read_csv('LIP_AXON/cell_metadata.csv',index_col=0)
else:
...
for i in all_files:
    ...
    if os.path.exists(f'check_fig/AXON/{file_name}.png'):
    ...
    if isinstance(check_info, int):
        ...
        try:
            ...
            fig.savefig(f'check_fig/AXON/{file_name}.png')
        except:
    ...
    counter+=1
    if counter>=5:
        pass_check_cell_meta.to_csv('LIP_AXON/cell_metadata.csv')
        counter=0
pass_check_cell_meta.to_csv('LIP_AXON/cell_metadata.csv')